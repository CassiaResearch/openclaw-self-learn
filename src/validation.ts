...
const ALLOWED_SUPPORTING_DIRS = new Set([
  "references",
  "scripts",
  "assets",
  "templates",
...
export function validateName(name: string): ValidationResult {
  ...
  if (!NAME_RE.test(name)) {
    return {
      valid: false,
      error:
        "Skill name must be lowercase alphanumeric with hyphens, starting with a letter (e.g., 'deploy-k8s-helm').",
      warnings,
    };
  }
...
export function validateFrontmatter(content: string): ValidationResult {
  ...
  if (endIdx === -1) {
    return {
      valid: false,
      error: "SKILL.md frontmatter must have a closing --- line.",
      warnings,
    };
  }
  ...
  if (!fm.name) {
    return {
      valid: false,
      error: "Frontmatter must include a 'name' field.",
      warnings,
    };
  }
  if (!fm.description) {
    return {
      valid: false,
      error: "Frontmatter must include a 'description' field.",
      warnings,
    };
  }
  if (fm.description.length > MAX_DESCRIPTION_LENGTH) {
    return {
      valid: false,
      error: `Description must be at most ${MAX_DESCRIPTION_LENGTH} characters (got ${fm.description.length}).`,
      warnings,
    };
  }
  if (fm.compatibility && fm.compatibility.length > MAX_COMPATIBILITY_LENGTH) {
    return {
      valid: false,
      error: `Compatibility must be at most ${MAX_COMPATIBILITY_LENGTH} characters (got ${fm.compatibility.length}).`,
      warnings,
    };
  }
  ...
  const unknownFields = Object.keys(fm).filter(
    (key) => !ALLOWED_FRONTMATTER_FIELDS.has(key),
  );
  ...
  if (!body) {
    warnings.push(
      "SKILL.md has no body content after frontmatter. Consider adding instructions.",
    );
  }
...
export function validateCreate(
  ...
  if (fm.name && fm.name !== name) {
    return {
      valid: false,
      error: `Frontmatter name '${fm.name}' must match the skill name '${name}'.`,
      warnings,
    };
  }
  ...
  if (skillExists(workspaceDir, name)) {
    return {
      valid: false,
      error: `A skill named '${name}' already exists. Use action='edit' to rewrite or action='patch' to update.`,
      warnings,
    };
  }
  ...
  if (currentSkills.length >= maxSkills) {
    return {
      valid: false,
      error: `Maximum skill limit (${maxSkills}) reached. Delete unused skills before creating new ones.`,
      warnings,
    };
  }
...
export function validateEdit(
  ...
  if (!skillExists(workspaceDir, name)) {
    return {
      valid: false,
      error: `Skill '${name}' does not exist. Use action='create' to create it first.`,
      warnings,
    };
  }
...
export function validatePatch(
  ...
  if (!find) {
    return {
      valid: false,
      error: "The 'find' parameter is required for patch action.",
      warnings,
    };
  }
...
export function validateFilePath(
  ...
  if (filePath.includes("..") || path.isAbsolute(filePath)) {
    return {
      valid: false,
      error: "File path must be relative and must not contain '..' segments.",
      warnings,
    };
  }
  ...
  if (!firstSegment) {
    return {
      valid: false,
      error: "File path must start with a subdirectory.",
      warnings,
    };
  }
  if (firstSegment === "SKILL.md") {
    return {
      valid: false,
      error: "Use action='edit' or action='patch' to modify SKILL.md directly.",
      warnings,
    };
  }
  ...
  if (!isPathInsideSkillsDir(fullPath, resolveSkillDir(workspaceDir, name))) {
    return {
      valid: false,
      error: "File path resolves outside the skill directory.",
      warnings,
    };
  }
